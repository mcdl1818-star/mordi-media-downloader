...
export function instagramConnectPage({ token = "", error = "", needsCode = false, success = false } = {}) {
  ...
  const content = success
    ...
    : `<h1>חיבור Instagram לבוט</h1><p>ההתחברות מתבצעת פעם אחת. הסיסמה נשלחת ישירות לתהליך ההתחברות ואינה נשמרת.</p>${message}<form method="post" action="/connect/instagram"><input type="hidden" name="token" value="${escapeHtml(token)}"><label>שם המשתמש של הפרופיל<input name="username" dir="ltr" placeholder="username או קישור לפרופיל" autocomplete="username" autocapitalize="none" autocorrect="off" spellcheck="false" required></label><small>לא אימייל ולא מספר טלפון. אפשר להדביק @username או קישור מלא לפרופיל.</small><label>סיסמת Instagram<input name="password" type="password" dir="ltr" autocomplete="current-password" minlength="6" maxlength="200" required></label>${code}<button type="submit">חבר את Instagram</button></form><small>מומלץ להשתמש בחשבון משני. אם Instagram תבקש אישור באפליקציה, אשר וחזור לכאן.</small>`;
